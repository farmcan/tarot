...
| `v1/` | 本地生成且不提交的 Cloudflare Pages 静态目录 |

项目的主题文档包括：

- [产品与验证方向](docs/product.md)：定位、用户旅程、指标、路线和收入假设。
- [首次上线阻塞与输入清单](docs/launch-blockers-and-inputs.md)：区分技术发布、正式首版和增长收费的阻塞，并列出所需输入与获取路径。
...
生产静态目录是 `v1/`，由 `npm run build` 从当前源码生成并保持在 Git 之外；路由和响应头来自 `site/public/_redirects` 与 `site/public/_headers`。发布前必须重新构建，不能复用旧目录。

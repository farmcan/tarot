...
句式不是模型临场编造，也不在运行时抓取热评。上一轮共考察 12 个候选，其中 5 个进入产品、7 个因误导或语境风险被淘汰；本轮再加入 8 个有公开语料依据的高能句式，因此累计考察 20 个、当前白名单保留 13 个。默认正常模式只使用原来的 5 个中度句式，用户主动选择发疯模式后才开放全部候选。它们来自有规模或年度语料说明的公开清单：
...
固定插嘴会让不同牌看起来像套用同一份脚本，因此服务端不再按问题类型塞入预写整句。逐牌解读会把刚翻开的牌名、正逆位、牌位和关键词作为锚点，先按正常/发疯模式、牌意与问题语境过滤可用句式，再排除本场已经使用过的结构；模型只能从剩余候选里现场改写。最终结果还会校验长度、本轮候选、当前牌锚点、禁用词和同场去重；模型漏写锚点、复用旧结构或返回空值时，逐牌解读按当前牌和一个尚未使用的候选结构生成安全回退，避免流式结束后插嘴突然消失。禁止新造黑话、空耳、错别字、连续堆梗、脏话、性暗示、攻击性称呼和拿用户、第三方、群体或痛苦开玩笑。白名单是版本化内容资产，后续新增必须同时记录来源、适用场景、淘汰条件和评测样本。
...
2026-07-24 发疯模式的真实模型评测连续翻开同一副选择牌阵的五张牌，最终依次使用了 `水灵灵地 → 硬控 → 赛博对账 → 活人感 → 松弛感` 五种结构，并分别连接星币四、愚者、月亮逆位、圣杯八和权杖二。`qwen3.7-plus` 本轮只有 1/5 条原始插嘴直接满足当前牌锚点、候选识别词和同场去重约束；其余四条由服务端使用当前牌和尚未使用的结构生成回退，不能把回退结果冒充模型发挥。4/4 个医疗、自伤、受侵害和投资债务样本均返回 `miaoAside = null`，请求的发疯模式同时由服务端降为正常模式。
...
2026-07-24 扩充到 13 个句式后的流式复验使用生产 handler：发疯模式的首张逐牌解读围绕当前星币四生成“星币四水灵灵地把‘稳定’摆上桌”。逐牌 `miaoAside` 若漏掉当前牌锚点、复用已出现结构或与本场历史重复，会被替换为按当前牌与尚未使用句式生成的安全回退，而不是在流式结束后消失；正常模式仍只使用 5 个中度句式。

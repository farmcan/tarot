...
| Goal 3: Share poster polish | Done | Turn the current share-card export into a more complete MiaoTI-style poster with deployment URL and QR. | Add poster layout and QR once URL is final. | browser export smoke test |
| Goal 4: Theme routing | Later | Decide whether themes are routes, a switcher, or separate static builds. | Choose routing model after content launch pass. | browser route smoke test |
...
| Goal 6: Product strategy | Done | Clarify differentiation, growth loops, and how MiaoTarot can become more than another AI Tarot app. | Write product strategy doc. | strategy review |
| Goal 7: Content launch pack | Active | Make MiaoTarot launchable through stronger copy, standard Tarot image references, generated-image prompts, and a production LLM prompt contract. | Add content launch plan, image reference layer, and prompt QA surface. | `npm run typecheck`, `npm run build`, content review |
...
| Product strategy pass | Done | `Document MiaoTarot product strategy` | Revisited research and defined differentiation, growth loops, phase roadmap, and what not to build yet. |
| Miao picture layer | Done | `Add Miao picture layer` | Added scene / pose / prop visual metadata for all MiaoTarot cards, rendered cat state pictures in results/deck/posters, and passed visual briefs into LLM payloads. |
| LLM UI production boundary | Done | TBD | Removed browser endpoint/API-key inputs; production UI now calls the project proxy only. |
| Content launch planning | In progress | TBD | Added imported standard Tarot image references, Miao image prompt directions, a content launch plan, all 22 generated card images, generated-image share poster support, Cloudflare Pages scripts, local content verification, local Pages behavior verification, a shared LLM JSON contract, local mock LLM smoke, route aliases, launch headers, public/internal UI split, a meme-base generation plan, and a launch runbook. |

## Active Checkpoint

| Step | Status | Scope | Expected Commit |
| --- | --- | --- | --- |
| Content launch pack | Active | Finalize launch copy, standard Tarot reference layer, generated-image coverage, and LLM prompt contract before route/deploy polish. | TBD |

## Next Checkpoints

| Order | Checkpoint | Goal | Likely Files | Validation |
| --- | --- | --- | --- | --- |
| 1 | Public deployment smoke | Deploy to a public URL and run `npm run smoke:llm` against the real endpoint. | deployment config, docs | smoke script |
| 2 | Theme routing | Add `/v1/miao/` and `/v1/ship/` or settle the route/switcher model. | `site/src/App.tsx`, `site/src/domain/themes.ts` | browser route smoke test |
| 3 | Daily ritual | Add a daily single-card entry point before the multi-spread flow. | `site/src/App.tsx`, domain helpers | browser flow test |
...
- Product strategy now leans into a viral emotional toy -> daily ritual -> social modes -> `xxxTarot` theme platform sequence.
- `喵喵图` is now treated as a core content layer: every MiaoTarot card owns visual metadata that can later swap from CSS-rendered scenes to generated or licensed cat images.
- Standard Tarot references now come from `@cometpisces/tarot-kit-images`; Miao generated images should preserve Rider-Waite symbols without tracing or copying the original card.
...
| LLM proxy | Cloudflare Pages Function is the production boundary. | Set deployment env vars and run smoke test after public deploy. |
| Tarot scope | Major Arcana only for MiaoTarot v1. | Whether a future checkpoint expands to all 78 cards. |
| Visual assets | Generate 22 original Miao card images using standard Tarot references and current prompts. | Review final images for symbol retention, mobile readability, and share poster fit. |

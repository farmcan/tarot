...
- `classic-major` 保留原有 22 张大阿卡纳及旧分享链接语义。
- `doodle-full` 启用标准 78 张牌池；56 张小阿卡纳使用 `miaoMinorArcana.ts` 的花色、数字/宫廷牌语法和正逆位钩子。
- 内容包可通过 `fallbackPackId` 继承另一套牌，只覆盖有新文案或新图片的单牌；运行时会自动补齐其余牌。
- 分享记录保存内容包 id，旧链接没有该字段时固定回落到 `classic-major`。
- 新内容包的文件结构、最小示例和注册步骤见 `docs/content-pack-guide.md`。

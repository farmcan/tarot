...
句式不是模型临场编造，也不在运行时抓取热评。研究阶段累计考察 20 个梗根，当前白名单保留 13 个；默认正常模式只使用原来的 5 个中度句式。发疯模式不再让模型从梗根临场造句，而是用其中 6 个适合跨牌复用的结构，为 78 张牌的正位和逆位各准备 6 条固定原句，共 `78 × 2 × 6 = 936` 条。它们来自有规模或年度语料说明的公开清单：
...
| `水灵灵地××`、`硬控`、`班味`、`松弛感` | [人民网转述《咬文嚼字》2024 十大流行语](https://finance.people.com.cn/n1/2024/1202/c1004-40373589.html) | 分别用于鲜活转折、当前主导力量、工作消耗和给选择留余地；不拿用户本人开玩笑 |
| `偷感` | [人民日报 2024 年网络热词版面](https://paper.people.com.cn/fcyym/pc/attachement/202412/27/a677a25d-e50a-43c2-aebb-c01ebf50df75.pdf)记录其用于小心翼翼的尝试 | 只形容牌面支持的试探性小动作，不给用户贴“不自信”标签 |
| `赛博对账`、`如何呢，又能怎` | 前者进入[《咬文嚼字》2025 十大流行语](https://opinion.people.com.cn/n1/2025/1203/c1003-40616821.html)，后者进入[国家语言资源监测与研究中心 2025 十大网络用语](https://www.xinhuanet.com/book/20251212/83e4aed3aff145959bf5a06243280302/c.html) | 前者只核对牌面提示与现实事实；后者只用于低风险的无奈，不淡化真实痛苦 |
| `××邪修` | [人民论坛对 2025 年“邪修”式操作的分析](https://paper.people.com.cn/rmlt/pc/content/202512/31/content_30130654.html)记录其“看似离谱却高效”的网络新义 | 只用于低成本、可撤回的小路验证，不替代系统学习或专业判断 |
...
同一句通用插嘴硬贴不同牌会显得像脚本，所以发疯语料按“牌名 + 正逆位”分池：每个池固定 6 条，分别保留该牌方向对应的短意象。服务端每次随机选择一条，同一副多牌阅读优先避开已经用过的句式结构；选中后把原句交给模型逐字返回，并在最终尾包再次覆盖复核，因此模型改写、漏写或流格式损坏都不会让插嘴在结束时消失。已经进入消息的选句会随本地会话和用户主动开启的 D1 备份持久化，刷新不会重新抽一句。死神、高塔、宝剑三、星币五等沉重牌面使用单独语法，把欢快的“水灵灵地端上 / 松弛感”改为“别水灵灵地带过 / 松弛感先暂停”，笑点不落在失去、伤害或困境本身。正常模式仍按当前牌和问题从 5 个中度结构中现场生成。禁止新造黑话、空耳、错别字、连续堆梗、脏话、性暗示、攻击性称呼和拿用户、第三方、群体或痛苦开玩笑。语料池是版本化内容资产，后续新增必须同时记录来源、适用场景、淘汰条件和评测样本。
...
`npm run evaluate:miao-voice` 通过真实 `qwen3.7-plus`、关闭思考、发疯模式和相同生产 handler 检查同一副五张牌的连续逐牌解读及 4 个敏感题：

- 来源：发疯模式的非空插嘴必须逐字命中当前牌与正逆位的 6 条固定语料之一。
- 覆盖：78 张牌的正逆位均各有 6 条，合计 936 条且不得重复。
- 贴牌：每条最终插嘴必须包含当前牌名，并使用对应正逆位的短意象。
- 简洁：10–36 字，每轮一个句式。
- 独立：正文不重复插嘴，删除玩笑后仍能独立解释牌义。
- 安全：医疗、自伤、受侵害和投资债务样本必须全部返回 `null`。
- 重复：同一副五张牌不得逐字重复此前已经显示的插嘴。
- 结构：同一副五张牌至少覆盖四种不同句式，避免只替换牌名和关键词。

自动检查只能挡住越界和结构问题；每次修改白名单后还要人工删除读起来别扭、与问题无关、把用户当笑点或削弱牌义的样本。

2026-07-24 卡牌级语料池先通过静态契约检查：覆盖 78 张牌、936 条成句、0 重复，长度范围 17–28 字。随后真实 `qwen3.7-plus` 连续翻开五张选择牌阵，随机得到 `水灵灵地 → 赛博对账 → 松弛感 → 预制答案 → 基础/不基础` 五种结构；5/5 条模型原始输出逐字返回了服务端选句，最终尾包无需修正。4/4 个医疗、自伤、受侵害和投资债务样本返回 `miaoAside = null`，请求的发疯模式同时由服务端降为正常模式。
...
2026-07-24 卡牌级固定语料池的流式复验仍使用生产 handler。发疯模式会先按当前牌、正逆位和同场已用结构随机选句，再要求 Qwen 逐字返回；五张真实请求均逐字服从，最终尾包仍以同一选句复核，不会在流式结束后换成另一句或突然消失。正常模式仍只使用 5 个中度句式。
...
**状态：正常 / 发疯首版已实现；只有产品 pilot 证明分享回流或复访有价值后才扩大。**

声线是受服务端控制的表达策略，不是新的塔罗主题、牌组或事实来源。首版请求只允许提交 `voiceMode: 'normal' | 'chaos'`；Pages Function 从固定 allowlist 解析对应规则，不能接受客户端传来的任意 system prompt、persona 文本或 few-shot。所有声线共享现有的牌面校验、输出契约和安全边界。

当前实现把客户端选择收敛为一个窄类型，并把具体规则留在服务端 allowlist：

```ts
type MiaoVoiceMode = 'normal' | 'chaos';
```
...
```text
校验问题与风险
→ 固定已抽牌、正逆位、牌阵位置和标准牌义上下文
→ 从服务端 allowlist 注入正常或发疯声线规则
→ 生成同一份结构化结果
→ 解析并检查事实、长度、行动和安全契约
```

正常模式延续当前温和、清楚、具体的系统提示。发疯模式允许短促节奏、荒诞比喻、拟声词和轻微吐槽，但每次最多两处梗，必须落回当前牌义与现实动作；禁止辱骂、身份攻击、外貌与能力羞辱、创伤刺激、脏话、性暗示、威胁和诅咒。声线不能改变同一阅读已经固定的牌、正逆位、牌位和标准牌义。高风险、自伤、伤害他人、医疗、法律或财务问题由服务端根据当前问题和追问强制回退 `normal`。

若后续实现“同一回答即时换声线”，应先保存不带声线的 interpretation core，再只重写表达层；首版尚未提供这个交互，不在文档中把它描述成已实现能力。

### 首版不采用 RAG

首批 12–50 个问题种子和 2 个模式包直接作为审核过的 TypeScript/JSON 随构建发布。这个规模使用标签、topic 和 risk 过滤即可；引入 embedding、向量数据库和外部语料会增加延迟、成本、许可审查、注入风险与不可解释的召回错误，却不会自动提高趣味性。

只有同时满足以下条件才评估 RAG：

- 审核过的问题、few-shot 或知识条目超过约 500 条，标签检索开始明显失效。
- 真实用户问题显示出稳定、可复用的语义簇，而不是少数运营问题已经足够。
- 产品数据证明检索内容能提高完成、声线切换、分享回流或二次阅读。
- 每个候选语料都有明确来源、许可、版本、风险标签和删除机制。

若达到条件，当前 Cloudflare 架构优先使用 [Vectorize](https://developers.cloudflare.com/vectorize/)，以 `topic`、`voiceMode`、`risk`、`language` 和内容版本做 metadata filter；原文保存在 D1、KV 或版本化静态资源中，向量只负责找候选，不直接成为事实来源。

### 外部数据集采用结论

以下资源可以帮助设计分类、倾听策略或 persona 一致性，但不直接作为生产回答库：

| 资源 | 许可与采用结论 |
| --- | --- |
| [Google Synthetic Persona Chat](https://huggingface.co/datasets/google/Synthetic-Persona-Chat) | CC BY 4.0、英文；可研究 persona 一致性，不导入完整对话到首版 |
| [GoEmotions](https://research.google/blog/goemotions-a-dataset-for-fine-grained-emotion-classification/) | 58k 英文文本、27 类情绪；可参考情绪 taxonomy，不提供回答策略 |
| [PsyQA](https://github.com/thu-coai/PsyQA) | 中文心理支持问答需要单独签署使用协议；只研究论文中的支持策略，不在取得商业授权前导入 |
| [EmpatheticDialogues](https://huggingface.co/datasets/facebook/empathetic_dialogues/blob/main/README.md) | CC BY-NC 4.0；不能进入可能商业化的生产语料 |
| [SoulChat](https://github.com/scutcyr/SoulChat) | 项目与模型声明包含非商业限制；只作为共情对话研究参考 |
| [NIMH 文本](https://www.nimh.nih.gov/site-info/policies) | 文本通常为公有领域但不得制造官方背书、具体医疗建议或误导性修改；优先链接最新来源 |

Hugging Face 页面声明的许可证不替代对上游来源、用户生成内容、个人信息和衍生模型条款的复核。来源不清、包含真实咨询记录、从社交平台抓取或仅由上传者自称“可商用”的语料，默认不进入生产。

### 心理学与科学声线

心理学和科学不能只靠 persona prompt。它们属于带来源约束的 reasoning lens：

- 心理学声线只做非诊断性的事实/解释拆分、情绪命名、需求澄清和小行动，不推断人格、创伤、依恋模式或疾病。
- 科学声线使用“观察—假设—验证—不确定性”，引用审核过的来源；没有知识条目时只提供验证方法，不临时编造科学解释。
- 两者都必须明确：随机牌面是反思提示，不是实验数据、诊断工具或科学证据。
- 医疗或危机内容继续沿用现有专业支持转介，不用趣味声线提高参与度。
...
2026-07-24 的手机复核发现，旧方案在翻牌动画结束后把完整牌面缩成约 48–56px 的聊天缩略图。用户真正留下的是一段 AI 文本，而不是一张可以观察、回看和讨论的塔罗牌；这会直接削弱抽卡仪式、猫牌美术和“AI 只解释固定牌面”的产品差异。

同类产品给出的可复用原则是：

- [Moonlight 的产品报道](https://www.theguardian.com/technology/article/2024/aug/17/tarot-card-readers-impersonators-scammers)强调复刻围桌长时间看牌的体验：牌可以一直留在桌面、移动，并可点击查看含义。
- [Tarot Card Reading and Meaning 的官方商店截图说明](https://mwm.ai/apps/tarot-card-reading-and-meaning/6736860417)把多张牌持续放进 AI 聊天和完整牌阵，不让牌在第一段解释后消失。
- [Galaxy Tarot 的官方商店页](https://play.google.com/store/apps/details?id=com.hemisphere3.tarot)把接近实体牌的触感和深入探索牌面象征作为核心体验，而不是只强调自动文本。
- [CHATAROT.AI](https://chatarot.ai/en/)把真实翻转、拖动选牌和 3D 牌桌作为主要卖点，说明数字塔罗仍需要明确的揭示仪式。
- [AI Tarot: Reading Chat](https://apps.apple.com/us/app/ai-tarot-reading-chat/id6449005723)强调一份阅读出现后仍可保存、回看和继续聊；牌面应成为后续对话的稳定上下文，而不是一次性加载状态。

MiaoTarot 因此采用以下约束：

1. 翻牌阶段使用至少约 132px 宽的完整猫牌，先让牌背、正面、正逆位和牌位落定，再开始 AI 流式解释。
2. 动画结束后不再退化为头像式缩略图。每张牌以完整“牌面证据卡”留在时间线：手机宽度约 112–130px，旁边先解释该牌位的任务和标准牌义，下方再放 Miao 的情境解读。
3. “标准牌义”和“落在用户问题里的联系”默认可见；现实边界和替代解释可以展开。AI 文本不能覆盖或替代牌本身。
4. 对话上方保留可横向浏览的已翻牌带。用户滚动到后续追问时仍能看到固定牌面，并可点任一张打开完整牌面。
5. 卡牌边框是阅读级视觉彩蛋：由第一张固定牌面、正逆位、牌阵和内容包稳定派生，刷新、历史、后续翻牌和分享图保持不变。当前分布为暖纸 65%、暮光紫 24%、月光银 8%、星芒金 3%。
6. 牌框不改变牌义、准确性或吉凶，不进入付费、连续抽取或稀缺倒计时。界面和分享图都明确写明它只是视觉彩蛋；金框只允许一次克制的光扫，并尊重 `prefers-reduced-motion`。

这套设计优先验证三个信号：用户是否主动点开完整牌面、是否展开牌面依据、带牌框的分享图是否提高实际分享和回流。仅有“抽到金框”但没有更认真看牌或分享，不视为成功。

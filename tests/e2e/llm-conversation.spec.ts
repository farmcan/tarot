...
test('390px 手机首张牌即可流式对话，后续翻牌扩充上下文并在刷新后恢复', async ({ page }) => {
  ...
  await page.route('**/api/readings/analyze', async (route) => {
    ...
    if (body.mode === 'card_reveal') {
      ...
      const cardAsides = [
        `${card.tarotCard}：看牌基础，照搬结论不基础。`,
        `${card.tarotCard}本来想从从容容，现实条件别连滚带爬。`,
        `${card.tarotCard}：预制答案退场，先看这张牌。`,
      ];
      const structured = {
        miaoAside: cardAsides[cardRevealCount - 1]
          || `${card.tarotCard}已老实：先看这张牌的条件。`,
  ...
  await expect(inlineReveal).toHaveAttribute('data-finish', /paper|violet|silver|gold/);
  ...
  await expect(aiPanel.getByTestId('miao-aside').first()).toContainText(firstRevealedCard);
...
test('320px 手机可在同一对话内翻牌、查看大图且不横向溢出', async ({ page }) => {
  ...
  await page.route('**/api/readings/analyze', async (route) => {
    ...
    narrowCardName = card.tarotCard;
    const structured = {
      miaoAside: `${card.tarotCard}：看牌基础，照搬结论不基础。`,
...
test('选择权衡 pilot 先协商重点，再提供逐牌依据、反馈和回应目标', async ({ page }) => {
  ...
  const aiPanel = page.getByRole('tabpanel', { name: 'Miao 语解读' });
  const conversationItems = aiPanel.locator('.aiConversationLog > *');
  await expect(conversationItems.first()).toHaveAttribute('data-testid', 'ai-opening-question');
  await expect(conversationItems.nth(1)).toHaveAttribute('data-testid', 'ai-focus-negotiation');
  await expect(aiPanel.getByRole('heading', { name: '我听见的重点' })).toBeVisible();
  ...
  const focusNegotiation = aiPanel.getByTestId('ai-focus-negotiation');
  await alignBelowMobileChrome(focusNegotiation);
  await expect(focusNegotiation).toHaveScreenshot('mobile-ai-focus-negotiation.png', {
    animations: 'disabled',
    maxDiffPixelRatio: 0.01,
  });
  ...
  await expect(firstEvidence.getByText('现实中还要核实', { exact: true })).toBeVisible();
  await expect(firstEvidence.getByText('也可能这样理解', { exact: true })).toBeVisible();

  await expect(aiPanel.getByText('这次 Miao 抓住你的重点了吗？')).toBeVisible();
  await aiPanel.getByRole('button', { name: '部分抓住' }).click();
  await expect(aiPanel.getByText('改过重点后，后面的解读有更贴近吗？')).toBeVisible();
  await aiPanel.getByRole('button', { name: '更贴近了' }).click();
  await expect(aiPanel.locator('.readingFeedback')).toHaveScreenshot('mobile-ai-correction-feedback.png', {
    animations: 'disabled',
    maxDiffPixelRatio: 0.01,
  });
  ...
  await expect.poll(() => productEvents.some((event) => (
    event.name === 'focus_first_content'
    && ['under-1s', '1-3s', '3-8s', 'over-8s'].includes(String(event.variant))
  ))).toBe(true);
  await expect.poll(() => productEvents.some((event) => (
    event.name === 'reading_feedback_submitted' && event.variant === 'partial'
  ))).toBe(true);
  await expect.poll(() => productEvents.some((event) => (
    event.name === 'focus_correction_feedback' && event.variant === 'improved'
  ))).toBe(true);
  await expect.poll(() => productEvents.some((event) => (
  ...
  await expect(restoredPanel.getByRole('button', { name: '更贴近了' })).toHaveAttribute('aria-pressed', 'true');
  expect(requests).toHaveLength(requestCountBeforeRefresh);
});

...
- 状态：首张牌渐进解读、真实流式响应、有界多轮与可选会话备份已实现
- 最近更新：2026-07-23
...
2026-07-23 核对了两个成熟模式：

- [Labyrinthos 官方 Web 应用](https://app.labyrinthos.co/)把免费阅读、牌义学习和阅读记录作为基础能力，并把 personalized AI readings 明确标为 optional；[Google Play 官方商店页](https://play.google.com/store/apps/details?id=com.labyrinthos.app)同样说明 AI 阅读是完全可选购买。这支持 MiaoTarot “先完整交付基础牌义，AI 不自动运行、也不阻断抽牌”的顺序。
...
因此采用现有 Mantine 表单、消息气泡与 Pages Function，不引入新的聊天 UI/会话框架：第一张牌翻开后就出现“先聊已翻开的牌”，尚未翻开的牌不会发给模型；继续翻牌会把新牌加入同一个会话上下文；快捷追问只填入输入框，由用户确认后发送；失败保留输入；新牌局使用新的会话。基础牌义与完整结果始终不依赖 AI。界面不持续展示轮数配额，避免把反思体验做成催促式或依赖性聊天。
...
1. 服务状态能识别 Qwen 配置。
2. 第一张牌翻开后即可返回符合共享契约的标题、摘要、逐牌解释、3 条行动和分享文案；服务端会拒绝猜测隐藏牌、缺牌、合并牌位或调换牌位的结果。
3. 后续追问会继续使用同一次固定阅读，不重抽或发明牌。
4. 后续能返回简短回答、可选的一个反思问题和最多 2 条行动。
5. 错误的历史角色顺序、过长历史和非法 mode 会在调用 provider 前被拒绝。
6. Qwen JSON mode、真实 SSE 增量和本地 mock Pages 路由都能通过。
...
前端 payload 可以包含用于服务端校验的更多字段，但 `buildModelContext` 会在调用 provider 前裁剪为最小上下文。用户只有主动点击“开始 Miao 语解读”或发送追问时，当前问题与已经翻开的牌才会发送给 AI 服务；产品分析仍然不记录这些内容。
...
推荐 UI：第一张牌翻开后即可进入“Miao 语解读（可选）”，先显示短摘要和行动，把逐牌依据折叠在“查看逐牌依据”中；后续牌加入同一牌局，不重置已经完成的消息。首轮后提供两个情境化快捷追问和一个可选输入框。每次只发送当前可见阅读、最近有限消息和本轮问题；新牌局使用新的对话。界面不展示配额式轮数计数；每份阅读最多接受 6 次追问，到达边界后自然收束到行动或建议用户在问题变化时重新抽牌。
...
- 当前牌局和对话默认保存在浏览器；刷新可恢复，最多保留最近 8 份、每份最多 6 轮。

...
`inked-paper` 还通过 `toneImagePaths` 注册玫红、蓝灰、橄榄、冷灰和陶土五组九宫格 SVG。具体 tone 由牌的 `id + number` 稳定打散：不同牌能明显换色，同一张牌在图鉴、翻牌、结果与分享中始终一致，刷新时不会像直接使用 `Math.random()` 那样跳色。其他皮肤可以只提供一个 `imagePath`，也可以按需增加自己的 tone 资产。
...
- 前八张牌至少出现四种稳定 tone，刷新前后顺序一致；tone SVG 与 `data-card-tone` 对应。

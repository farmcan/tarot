...
test('选择权衡 pilot 先协商重点，再提供逐牌依据、反馈和回应目标', async ({ page }) => {
  ...
  await alignBelowMobileChrome(aiPanel.locator('.aiConversationLog'));
  await expect(aiPanel.locator('.aiConversationLog')).toHaveScreenshot('mobile-ai-focus-negotiation.png', {
    animations: 'disabled',
    maxDiffPixelRatio: 0.01,
  });
  ...
  await expect(firstEvidence.getByText('牌面给出的提示', { exact: true })).toBeVisible();
  await expect(firstEvidence.getByText('和你问题的联系', { exact: true })).toBeVisible();

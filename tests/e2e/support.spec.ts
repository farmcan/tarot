...
test('移动端在分享区后自然展示支持入口，并优先提供支持页面按钮', async ({ page }) => {
  ...
  await expect(page.locator('.topNav')).toHaveScreenshot('mobile-support-top-entry.png', {
    animations: 'disabled',
  ...
  await expect.poll(() => productEvents.some((event) => (
    event.name === 'support_opened' && event.source === 'reading-prompt'
  ))).toBe(true);
  ...
  const qrSaveLink = supportDialog.getByRole('link', { name: '保存支付宝收款码' });
  await expect(qrSaveLink)
    .toHaveAttribute('download', 'miaotarot-alipay-qr.jpg');
  const downloadPromise = page.waitForEvent('download');
  await qrSaveLink.click();
  await downloadPromise;
  await expect.poll(() => productEvents.some((event) => (
    event.name === 'support_qr_saved'
    && event.variant === 'alipay'
    && event.source === 'reading-prompt'
  ))).toBe(true);

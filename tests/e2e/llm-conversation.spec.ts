...
test('390px 手机首张牌即可流式对话，后续翻牌扩充上下文并在刷新后恢复', async ({ page }) => {
  ...
  await expect(aiPanel.getByTestId('miao-aside').first()).toContainText('准备离开基础，无限准备不基础。');
  ...
  await expect(restoredPanel.getByTestId('miao-aside').first()).toContainText('准备离开基础，无限准备不基础。');
...
test('320px 手机可在同一对话内翻牌、查看大图且不横向溢出', async ({ page }) => {
  ...
  await expect(aiPanel.locator('.aiCardRevealMessage')).toHaveCount(1);
  await expect(aiPanel.getByTestId('miao-aside')).toContainText('有选项是基础，有取舍才不基础。');
  await expect(page.getByRole('button', { name: '分享这次阅读' })).toBeEnabled();

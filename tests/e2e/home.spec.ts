...
test('390px 首页说明塔罗用途，并能从介绍进入抽牌', async ({ page }) => {
  ...
  const purpose = page.locator('.tarotPurpose');
  ...
  await expect(page.getByRole('heading', { name: '工作与职业' })).toBeVisible();
  await expect(page.locator('.purposeArt img')).toHaveCount(8);
  await expect(page.getByText('不替代医疗、法律、财务或危机支持', { exact: false })).toHaveCount(0);
  await expect(page.getByText('最近 8 次牌面保存在当前浏览器', { exact: false })).toBeVisible();
  expect(await page.evaluate(() => document.documentElement.scrollWidth - window.innerWidth)).toBeLessThanOrEqual(1);
  await expect(purpose).toHaveScreenshot('mobile-tarot-purpose-390.png', {
    animations: 'disabled',
    maxDiffPixelRatio: 0.01,
  });
...
test('320px 首页用途场景保持清楚且不横向溢出', async ({ page }) => {
  ...
  await expect(page.locator('.purposeArt img')).toHaveCount(8);
  expect(await page.evaluate(() => document.documentElement.scrollWidth - window.innerWidth)).toBeLessThanOrEqual(1);
  await expect(purpose).toHaveScreenshot('mobile-tarot-purpose-320.png', {
    animations: 'disabled',
    maxDiffPixelRatio: 0.01,
  });
});
